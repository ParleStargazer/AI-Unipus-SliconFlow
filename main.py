...
def main():
    ...
    - OutputFormat: 不使用markdown语法, 使用一般文字输出; 根据题型返回不同格式的答案及原因, 最后以形如["","","",""]的格式汇总输出所有答案, 最外围以一对[]包括整个答案, 内部每题用""围起来, 并以单个','分隔每道题目, 注意输出格式的准确性, 不要有非法数据与格式, 不要有无效冗余数据, 注意填空题答案不要包含周边的题目
    - AnswerFormat: 单选题: "A", 多选题: "A,B,C,D", 填空题: "now answer", 回答题: "now reply"
    - Workflow:
    1. 仔细阅读并理解题目内容
    2. 针对每个题目, 分析选项与文章内容的关系(如果有选项)
    3. 根据文章内容和逻辑推理, 确定每个问题的正确答案, 并解释为什么
    4. 在解释完所有问题后, 以形如["","","",""]的格式汇总输出所有答案, 最外围以一对[]包括整个答案, 内部每题用""围起来, 并以单个','分隔每道题目, 注意输出格式的准确性, 不要有非法数据与格式, 不要有无效冗余数据, 注意填空题答案不要包含周边的题目; 注意每种题型的回答格式: 单选题: "A", 多选题: "A,B,C,D", 填空题: "now answer", 回答题: "now reply"
    5. 注意最外围以一对[]包括整个答案, 多选题的选项是仅以逗号分隔且存放在仅一对引号中的"""
        ...
        KIMIResponse = KimiClient.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "欢迎来到英语文章解析与逻辑推理的世界。请提供一篇英语文章和一些题目, 我将为你解答这些题目并解释; 最后, 我会以形如["","","",""]的格式汇总输出所有答案, 最外围以一对[]包括整个答案, 内部每题用""围起来, 并以单个','分隔每道题目, 注意输出格式的准确性, 不要有非法数据与格式, 不要有无效冗余数据, 注意填空题答案不要包含周边的题目"},
